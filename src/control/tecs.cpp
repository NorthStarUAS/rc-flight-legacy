...
static void init_tecs() {
    ...
    if ( tecs_config_node.getFloat("mass_kg") < 0.01 ) {
        tecs_config_node.setFloat("mass_kg", 2.5);
    }
    if ( ! tecs_config_node.hasChild("weight_bal") ) {
        tecs_config_node.setFloat("weight_bal", 1.0);
    }
    tecs_inited = true;
}
...
void update_tecs() {
    ...
    double mass_kg = tecs_config_node.getFloat("mass_kg");
    double wb = tecs_config_node.getFloat("weight_bal");

    // Current energy
    double alt_m = pos_node.getFloat("altitude_agl_m");
    double vel_mps = vel_node.getFloat("airspeed_smoothed_kt") * SG_KT_TO_MPS;
    double energy_pot = mass_kg * g * alt_m;
    double energy_kin = 0.5 * mass_kg * vel_mps * vel_mps;
    tecs_node.setFloat("energy_pot", energy_pot);
    tecs_node.setFloat("energy_kin", energy_kin);
    
    // Target energy
    double target_alt_m = targets_node.getFloat("altitude_agl_ft") * SG_FEET_TO_METER;
    double target_vel_mps = targets_node.getFloat("airspeed_kt") * SG_KT_TO_MPS;
    ...
    tecs_node.setFloat("target_total", target_total);
    tecs_node.setFloat("target_pot", target_pot);
    tecs_node.setFloat("target_kin", target_kin);

    // Energy error
    double error_pot = target_pot - energy_pot;
    double error_kin = target_kin - energy_kin;
    tecs_node.setFloat("error_pot", error_pot);
    tecs_node.setFloat("error_kin", error_kin);
    
    // Compute min & max kinetic energy allowed (based on configured
    // operational speed range)
    double min_kt = tecs_config_node.getFloat("min_kt");
    if ( min_kt < 15 ) { min_kt = 15;}
    double min_mps = min_kt * SG_KT_TO_MPS;
    double min_kinetic = 0.5 * mass_kg * min_mps * min_mps;

    double max_kt = tecs_config_node.getFloat("max_kt");
    ...
    tecs_node.setFloat("error_diff", error_diff);
}
