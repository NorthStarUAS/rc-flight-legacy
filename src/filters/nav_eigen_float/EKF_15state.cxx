...
NAVdata EKF15_float::init(IMUdata imu, GPSdata gps) {
    ...
    quat = eul2quatf(nav.phi, nav.the, nav.psi);
    nav.qw = quat.w();
    nav.qx = quat.x();
    nav.qy = quat.y();
    nav.qz = quat.z();
...
NAVdata EKF15_float::update(IMUdata imu, GPSdata gps) {
    ...
    Vector3f att_vec = quat2eulf(quat);
    nav.phi = att_vec(0);
    nav.the = att_vec(1);
    nav.psi = att_vec(2);
	...
	pos_ref(2) = 0.0;
	pos_ins_ned = ecef2nedf(pos_ins_ecef, pos_ref);
	...
	pos_gps_ned = ecef2nedf(pos_gps_ecef, pos_ref);
