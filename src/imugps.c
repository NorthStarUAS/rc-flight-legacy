...
void *imugps_acq(void *thread_id)
{
    ...
    byte CH_BAUD[11]
        = { 0x55, 0x55, 0x57, 0x46, 0x01, 0x00, 0x02, 0x00, 0x03, 0x00, 0xA3 };
    byte CH_SAMP[11]
        = { 0x55, 0x55, 0x53, 0x46, 0x01, 0x00, 0x01, 0x00, 0x02, 0x00, 0x9D };
    byte CH_SERVO[7]
    ...
    while ( 1 ) {
        ...
        headerOK = 0;
        while ( 1 != read(sPort0, &input_buffer[2], 1) );
        nbytes = 3; 
        ...
        case 'N':               // IMU packet with GPS
            ...
            if ( checksum(input_buffer,FULL_PACKET_SIZE) == TRUE ) {
                ...
                if ( input_buffer[31] == (byte)0x55
                     && input_buffer[32] == (byte)0x55
                     && input_buffer[33]=='G')
                {
